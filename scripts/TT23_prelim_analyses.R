...
##############################################################################
## Vcmax regressed against subplot GM density and canopy openness
##############################################################################
...
facet.labels <- c("Open canopy", "Closed canopy")
names(facet.labels) <- c("pre_closure", "post_closure")

## Create plot
vcmax.cont.plot <- ggplot(data = subset(df, spp != "Ari"), 
                    aes(x = total_subplot, y = vcmax25, fill = spp)) +
  geom_point(size = 3, alpha = 0.75, shape = 21) +
  geom_ribbon(data = vcmax.trend,
              aes(y = response, ymax = upper.CL, ymin = lower.CL,
                  fill = spp), alpha = 0.3) +
  geom_smooth(data = vcmax.trend, method = 'lm', se = FALSE,
              aes(y = response, linetype = linetype, color = spp)) +
  facet_wrap(.~canopy, labeller = as_labeller(facet.labels)) +
  scale_linetype_manual(values = c("dashed", "solid")) +
  scale_y_continuous(limits = c(0, 200)) +
  labs(x = "Total garlic mustard density", 
       y = expression(bold("V"["cmax25"]*" ("*mu*"mol"*" m"^"-2"*" s"^"-1"*")")),
       fill = "Species", color = "Species") +
  theme_bw(base_size = 18) +
